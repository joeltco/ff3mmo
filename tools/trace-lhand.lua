-- FCEUX Lua: trace L-HAND weapon positions ONLY (dagger in L, nothing in R)
...
-- Output: tools/weapon-lhand-trace.txt

local OUTPUT = "/home/joeltco/projects/ff3mmo/tools/weapon-lhand-trace.txt"
...
function finish()
  ...
  log("=== DONE ===")
  f:close()
  emu.message("Done! weapon-lhand-trace.txt — exiting FCEUX")
  os.exit(0)
end
...
function equip_weapons()
  -- R hand: nothing (0x00), L hand: Dagger (0x1F)
  for c = 0, 3 do
...
function on_frame()
  ...
  if state == "pressing" then
    ...
    if mode == 0x20 then
      state = "battle"
      battle_frames = 0
      log(string.format("[%d] BATTLE START — L-HAND ONLY (dagger, no R-hand weapon)", frame))
      log("Columns: pose# | bf | swing-type | hflip | weapon_x weapon_y | body_x body_y | offset_x offset_y")
      log("")
      emu.message("In battle! Tracing L-hand weapon positions...")
    end
    last_mode = mode
    return
  end
  ...
  if state == "battle" then
    ...
    if has_wpn then
      ...
      if pose_key ~= last_pose_key then
        ...
        log(string.format("L-HAND POSE #%d  bf=%-4d  %s  hflip=%s  weapon x=%-3d y=%-3d  body x=%-3d y=%-3d  offset_x=%-4d offset_y=%-3d",
...
log("=== L-HAND ONLY trace — dagger in L, nothing in R ===")
log("")
