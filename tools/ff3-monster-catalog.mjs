...
L.push('');
L.push('## ⭐ Byte 15 — copied, and never read');
L.push('');
L.push('Settled by **differential execution** (`tools/ff3-diff-trace.mjs`) after two');
L.push('weaker instruments gave two wrong answers. Two machines differing in exactly');
L.push('one ROM byte, identical input, compared instruction by instruction: anything');
L.push('that uses the value must diverge, in control flow if it branched or in a');
L.push('register if it computed. No knowledge of where the value lives is needed —');
L.push('which is precisely what defeated the earlier attempts.');
L.push('');
L.push('Byte 15 = 0 vs 255, compared from the **first frame** so setup is included,');
L.push('~22.1M instructions:');
L.push('');
L.push('```');
L.push('control flow parted:   NEVER');
L.push('registers differed at: $A5EF x2  $A5F1 x2  $A5F3 x2  $A5F4 x2');
L.push('```');
L.push('');
L.push('Those four PCs are the setup copy and nothing else — `LDA ($24),Y` puts byte');
L.push('15 in A at `$A5EE`, `STA ($5D),Y` writes it at `$A5F2`, and the difference');
L.push('vanishes at `$A5F5` where A is overwritten. **×2 because the encounter has two');
L.push('Goblins**: it is the per-combatant copy. The value is written into the');
L.push('combatant entry and then never loaded by anything, on either code path.');
L.push('');
L.push('**The negative is calibrated**, which is the only reason it counts. The same');
L.push('tool pointed at atkElem (0 vs FIRE, fire-resist shield) parts control flow');
L.push('mid-battle at `$A00D` and shows the value flowing through `$9F43`-`$9F4F`.');
L.push('⛔ It only passes that control with the monster making PHYSICAL attacks —');
L.push('set spAtkRate to 0xFF and it casts every turn, never swings, atkElem is never');
L.push('consulted, and the control silently fails.');
L.push('');
L.push('⛔ **Scope:** a full Goblin encounter, setup included, both paths. A path not');
L.push('exercised here — another monster\'s script, a scan effect, a bestiary screen —');
L.push('could still read it. What is established is that nothing in an ordinary');
L.push('battle does.');
L.push('');
L.push('### The two wrong answers, and why');
L.push('');
L.push('- v1.8.67 — "a dedicated reader at `$A5F3`". `$A5F3` is the second byte of the');
...
L.push('  also perform a spurious read at the un-carried address when they cross a');
L.push('  page, so that was an addressing artifact too.');
L.push('');
L.push('The lesson under all three: an address-watching hook reports the CPU\'s **bus');
L.push('traffic**, which includes reads no instruction semantically makes.');
L.push('Differential execution reports what the program actually **depends on**.');
L.push('');
