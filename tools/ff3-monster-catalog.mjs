...
L.push('');
L.push('## ⛔ Byte 15 — still unknown, and the last explanation was wrong');
L.push('');
L.push('v1.8.67 claimed byte 15 had "a dedicated reader at `$A5F3` that touches no');
L.push('other field". **`$A5F3` is not a reader.** Disassembling it — the sequence is');
L.push('unique in the ROM, file `0x625FC`, bank 49 — shows the battle-setup copy:');
L.push('');
L.push('```');
L.push('$A5EC  A0 0F     LDY #$0F        ; record offset 15');
L.push('$A5EE  B1 24     LDA ($24),Y     ; A = monster record byte 15');
L.push('$A5F0  A0 36     LDY #$36        ; combatant-entry offset $36');
L.push('$A5F2  91 5D     STA ($5D),Y     ; ...store it');
L.push('```');
L.push('');
L.push('`$A5F3` is the second byte of that `STA`, and the "read" a RAM hook sees there');
L.push('is the **dummy read cycle** an indirect-indexed store performs on its target.');
L.push('It is the code that PUTS byte 15 into RAM, not code that uses it.');
L.push('');
L.push('The v1.8.67 cross-check goes with it: `$A61D` and `$A65C` were cited as');
L.push('reading atkElem/elemResist/statusResist, "so the code agrees with the labels".');
L.push('They are `$A61C` and `$A65B`, both `STA ($5D),Y` — the same setup copy. They');
L.push('group by which fields are written together, not by what consumes them.');
L.push('');
L.push('**The method also fails its own positive control.** atkElem provably halves');
L.push('damage against a fire-resisting shield, yet tracing its RAM home with that');
L.push('effect active still shows no genuine load except a bulk block-move and one');
L.push('coincidental hit from a map routine. Whatever consumes atkElem reads it from');
L.push('somewhere else — so "nothing reads byte 15" is not a conclusion that is');
L.push('available, because the instrument cannot find a consumer known to exist.');
L.push('');
L.push('Byte 15 is simply still unknown: no behavioural effect across its whole');
L.push('range, and the code-side approach inconclusive until it passes its control.');
L.push('Recorded as unknown, **not** filled in from a wiki.');
L.push('');
