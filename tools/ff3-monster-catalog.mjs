...
L.push('');
L.push('### ⭐ Byte 15 is PACKED: a table index AND a drop rate');
L.push('');
L.push('Bits 0-4 are the steal/drop table index. Bits 5-7 are the **drop rate**,');
L.push('compared against a random 0..6 — so the chance is `rate/7`:');
L.push('');
L.push('```');
L.push('$BC7E  LDA $7412        ; the record\'s byte 15');
L.push('$BC81  JSR $FD44        ; = LSR A x5   (>> 5)');
L.push('$BC84  STA $2E');
L.push('$BC86  LDA #$06 / JSR $A564 / CMP $2E / BCS   ; roll >= rate -> nothing');
L.push('```');
L.push('');
L.push('| rate | chance | monsters |');
L.push('|---|---|---|');
L.push('| 0 | never | 180 |');
L.push('| 1 | 14.3% | 49 |');
L.push('| 7 | **always** | 3 — the dragons |');
L.push('');
L.push('⭐ So the dragons\' Onion gear is a **guaranteed** drop, which is the real');
L.push('shape of the famous farm. Verified by patching a spawnable monster to each');
L.push('rate: `0xFE` (rate 7) dropped 6/6 and yielded OnionArmor and Onion Helm;');
L.push('`0x20` (rate 1) dropped 2/6; `0x0A` (rate 0, non-empty table) dropped 0/6.');
L.push('');
