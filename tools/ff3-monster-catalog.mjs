...
L.push('offset `$36` through a pointer. Exactly one reads it — bank 53, `$8BE0`,');
L.push('file `0x6ABF0`:');
L.push('');
L.push('```');
L.push('$8BDE  A0 36     LDY #$36        ; entry offset $36 = byte 15');
L.push('$8BE0  B1 70     LDA ($70),Y     ; from the TARGET combatant');
L.push('$8BE2  29 1F     AND #$1F        ; low 5 bits only');
L.push('$8BE4  85 18     STA $18         ; ...as an index');
...
L.push('test.');
L.push('');
L.push('⛔ **Which ability owns that routine is not determined.** It is entered by');
L.push('fall-through; no JSR, JMP or RTS-dispatch pointer to it exists in banks');
L.push('48-56. Naming it would be a guess.');
L.push('');
