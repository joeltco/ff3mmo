...
L.push('');
L.push('## ⭐ Byte 15 — a graphics/table index, read by exactly one routine');
L.push('');
L.push('Two searches, each answering half the question.');
L.push('');
L.push('**No monster script reads it.** `tools/ff3-diff-trace.mjs` runs two machines');
L.push('differing in exactly this byte on identical input and compares every');
L.push('instruction — anything using the value must diverge in control flow or a');
L.push('register. Run from the first frame so setup is included, ~22.1M instructions');
L.push('each, across **eight monsters** chosen for script diversity (spAtkIdx');
L.push('0/2/7/8/15/23/24/43, rates 0-99):');
L.push('');
L.push('```');
L.push('control flow parted:   NEVER, for any of them');
L.push('registers differed at: $A5EF $A5F1 $A5F3 $A5F4 — the setup copy, x2');
L.push('```');
L.push('');
L.push('**But code that reads it exists.** A dynamic trace only sees paths that ran,');
L.push('so the ROM was scanned statically for every instruction able to reach entry');
L.push('offset `$36` through a pointer. Exactly one reads it — bank 53, file');
L.push('`0x6ABF0`. ⛔ v1.8.70 printed its CPU address as `$8BE0`; that was wrong by');
L.push('`$2000` — bank 53 sits in the `$A000` window, proven because the routine\'s');
L.push('own `JMP $ABB7` and `JSR $AB66` only land on instruction boundaries under');
L.push('that mapping. It is `$ABE0`:');
L.push('');
L.push('```');
L.push('$ABDE  A0 36     LDY #$36        ; entry offset $36 = byte 15');
L.push('$ABE0  B1 70     LDA ($70),Y     ; from the TARGET combatant');
L.push('$ABE2  29 1F     AND #$1F        ; low 5 bits only');
L.push('$ABE4  85 18     STA $18         ; ...as an index');
L.push('       ...pointer $9B80, count 8, JSR $FDA6   ; a loader');
L.push('```');
L.push('');
L.push('⭐ **`AND #$1F` is corroborated by the data**, which is what makes this more');
L.push('than a plausible reading: 179 of 232 monsters have byte 15 = 0, and every');
L.push('nonzero value is either 0x20-0x2E or 0xFD/0xFE/0xFF. Masked, those become');
L.push('0x00-0x0E and 0x1D-0x1F. The low 5 bits are the payload and bit 5 is a');
L.push('separate flag — a magnitude would not cluster like that.');
L.push('');
L.push('It is guarded, which is why no ordinary battle reaches it: a random roll');
L.push('against a threshold built from the attacker\'s stats, then a target-status');
L.push('test.');
L.push('');
L.push('⛔ **Which ability reaches it is still not determined**, after a real');
L.push('attempt. The routine starts at `$AB9F` and IS an entry in a pointer table in');
L.push('bank 52 (entry at file `0x69A42`), whose siblings are handlers in banks 52');
L.push('and 53 with `$9A68` repeating as a default — the shape of an effect');
L.push('dispatch. It reads offset `$36` from `($70)`, the **target**, so a monster');
L.push('attacking the party reads a PARTY member\'s byte; the monster\'s byte 15 is');
L.push('only readable when the monster IS the target. It is guarded three ways:');
L.push('target entry+`$2C` bit 7 set, an RNG roll against the attacker\'s stats, and');
L.push('target status bits clear.');
L.push('');
L.push('⛔ **And it never executes** — verified with `tools/ff3-pc-probe.mjs` across');
L.push('physical battles, monster specials, party magic, all 23 spell effect-types');
L.push('forced onto a castable spell, and 8 different monsters.');
L.push('');
L.push('⛔ A warning about the obvious way to measure that: counting PC hits WITHOUT');
L.push('checking the mapped bank reported 48 executions of `$AB9F` and 20 of');
L.push('`$ABDE`. Verified against the opcode bytes, both are **zero** — every hit');
L.push('was a different bank\'s code at the same CPU address.');
L.push('');
L.push('### Two earlier answers, and why they were wrong');
L.push('');
L.push('- v1.8.67 — "a dedicated reader at `$A5F3`". That is the second byte of the');
L.push('  `STA` that **writes** it; a RAM hook counts the dummy read cycle an');
L.push('  indirect-indexed store performs on its target.');
L.push('- v1.8.68 — "`$AA06 LDA ($82),Y` genuinely loads it". Indirect-indexed *loads*');
L.push('  also perform a spurious read at the un-carried address when crossing a page.');
L.push('');
L.push('An address hook reports the CPU\'s **bus traffic**, including reads no');
L.push('instruction semantically makes. Differential execution reports what the');
L.push('program **depends on**. A static scan reports what **could** run but did not.');
L.push('Byte 15 needed the last two together.');
L.push('');
