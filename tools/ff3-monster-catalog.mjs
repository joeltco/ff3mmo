...
L.push('');
L.push('### ⛔ The shipped hand-maintained data disagrees');
L.push('');
L.push('`src/data/monsters.js` carries `steal:` and `drops:` fields that');
L.push('`gen-monsters-js.js` "preserves from previous manual data". The game reads');
L.push('them, so the disagreement is behavioural, not cosmetic');
L.push('(`node tools/ff3-drop-audit.mjs`):');
L.push('');
L.push('| field | agrees with the ROM entry | names an item NOT in it | absent |');
L.push('|---|---|---|---|');
L.push('| `steal:` | 184 | 35 | 12 |');
L.push('| `drops:` | 20 | 30 | 181 |');
L.push('');
L.push('52 monsters name an item their ROM entry does not contain, and 16 of the');
L.push('agreeing ones list fewer items than the entry offers.');
L.push('');
L.push('⭐ The starkest case is the **dragons**, whose hand data has no `drops:` at');
L.push('all and a `steal:` of Potion — while the ROM entry is Elixir x4 plus the full');
L.push('**Onion equipment**. As shipped, FF3\'s most famous farm is not reachable.');
L.push('');
L.push('⛔ Recorded, not "fixed": ff3mmo is its own game, so whether to follow ROM');
L.push('canon here is a design call.');
L.push('');
