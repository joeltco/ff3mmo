...
const LISTA_TABLE = `| byte 0 | battles | n rolls | fires | measured (battle-clustered) | exact from ROM table | z |
|---|---|---|---|---|---|---|
...
lines.push('a read of `+2..+9` is a pass. Isolator is **LICH `0x77`, pool `$22`**: list A is');
lines.push('fully populated (`1F 1C 1D 16 15 14 0F 05`, no `$FF`) so there is no retry path');
lines.push('and one list read == one fire, and `byte 1 = 00` keeps list B inert.');
lines.push('⭐ Error bars are **battle-clustered** — each battle contributes one rate and the');
lines.push('spread ACROSS battles sets the SE. 10 battles x 30 rounds per row:');
lines.push('');
lines.push(LISTA_TABLE);
lines.push('');
lines.push('**Every row is consistent with the exact ROM-table count, |z| <= 0.40.**');
lines.push('Integrity across all 506 rolls: **506/506** logged bytes equal `TABLE[idx]` read');
lines.push('from the ROM, and **506/506** fire outcomes equal `floor(v*129/256) < chance`.');
lines.push('**The rate is `chance / 128`.**');
lines.push('');
lines.push('⛔ **This supersedes the v1.9.10 table** (`13.9 / 18.8 / 50.6 / 77.9 / 99.1%` over');
lines.push('14 battles). Those measurements were sound — the config had no retry path — but');
lines.push('they carried per-roll error bars, and on that basis `$20` was reported as');
lines.push('"1.5 sigma low". Re-measured with clustered bars it is **23.6% ± 5.2pp vs 25.39%,');
lines.push('z = -0.35** — never discrepant at all. The apparent outlier was an artifact of');
lines.push('the error bar, not of the game.');
lines.push('');
lines.push('⚠ **Clustering is NOT a fixed inflation — do not reuse a single factor.** The');
lines.push('byte 1 run measured ~1.9x, and an earlier draft of this section extrapolated that');
lines.push('number here. Measured per row, list A ranges **0.9x to 1.3x**: `$10` 1.3x wider,');
lines.push('`$20` 1.0x, and `$40`/`$60` actually NARROWER than binomial (underdispersed at');
lines.push('10 battles). The dispersion depends on the row, so each curve must be clustered');
lines.push('from its own data rather than scaled by a borrowed constant.');
lines.push('');
