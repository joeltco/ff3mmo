-- FCEUX Lua: trace R-HAND weapon positions ONLY (knife in R, nothing in L)
-- Tries save state slot 0 first; falls back to title screen navigation.
-- Calls os.exit(0) when done — kills FCEUX process (safe for headless/SSH use).
-- Output: tools/weapon-rhand-trace.txt

local OUTPUT = "/home/joeltco/projects/ff3mmo/tools/weapon-rhand-trace.txt"
...
function finish()
  ...
  log("=== DONE ===")
  f:close()
  emu.message("Done! weapon-rhand-trace.txt — exiting FCEUX")
  os.exit(0)
end
...
function equip_weapons()
  -- R hand: Knife (0x1E), L hand: nothing (0x00)
  for c = 0, 3 do
    local base = 0x6200 + c * 0x40
    memory.writebyte(base + 0x03, 0x1E)  -- right: Knife
    memory.writebyte(base + 0x05, 0x00)  -- left:  empty
  end
end
...
function scan_oam()
  ...
  for i = 0, 63 do
    ...
    if sy > 0 and sy < 240 then
      ...
      -- Head/torso tiles t=$01-$06 palette 0 — fixed body reference
...
function on_frame()
  ...
  -- ── TITLE nav (fallback when save state unavailable) ────────────────────────
  if state == "title" then
    if mode == 0xA8 then
    ...
    if name_timer > 620 and mode ~= 0xA8 then
      state = "pressing"
      log(string.format("[%d] Past name entry — pressing into battle", frame))
  ...
  -- ── PRESSING (wait for battle mode $20) ─────────────────────────────────────
  if state == "pressing" then
    ...
    if mode == 0x20 then
      state = "battle"
      battle_frames = 0
      log(string.format("[%d] BATTLE START — R-HAND ONLY (knife, no L-hand weapon)", frame))
      log("Columns: pose# | bf | swing-type | hflip | weapon_x weapon_y | body_x body_y | offset_x offset_y")
      log("")
      emu.message("In battle! Tracing R-hand weapon positions...")
    end
    last_mode = mode
    return
  end
  ...
  if state == "battle" then
    ...
    if has_wpn then
      ...
      local pose_key = swing .. w_x .. "_" .. body_x
      ...
      if pose_key ~= last_pose_key then
        ...
        log(string.format("R-HAND POSE #%d  bf=%-4d  %s  hflip=%s  weapon x=%-3d y=%-3d  body x=%-3d y=%-3d  offset_x=%-4d offset_y=%-3d",
...
log("=== R-HAND ONLY trace — knife in R, nothing in L ===")
log("")

local ok = pcall(function() emu.loadstate(0) end)
if ok then
  state = "pressing"
  log("[Loaded save state slot 0 — skipping title]")
  emu.message("Save state loaded — heading to battle!")
else
  state = "title"
  log("[No save state slot 0 — navigating from title screen]")
