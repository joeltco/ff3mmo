...
Kicks off the server-arbitrated PvP rewrite project. Full plan + wire contract: `docs/PVP-REWRITE-PLAN.md`. The inventory mirror project (v1.7.740-746) was the prerequisite — server-canonical equipped stats give the rewrite a single source of truth for combatant spawning that didn't exist before.

**Rules locked (per user spec):**
- Each side: 1 human-controlled main player + 0-3 AI-controlled party members (server-side AI, deterministic from mirror stats).
- Matchmaking: keep existing `pvp-search`-and-hook (challenger searches, server hooks opponent's next random encounter via AGI-differential roll).
- Party gating: any size — match what each player has online (solo vs full party allowed).
- Refactor: new `pvp-arbiter` module behind `PVP_ARBITER` flag; old `pvp.js` lockstep code stays for rollback until post-flip cleanup.
- UI: enemy name strip displays only the current attacker / target, not a static enemy list.

**P-0 — design doc** (this version):
- `docs/PVP-REWRITE-PLAN.md` — architecture, wire shapes (`pvp-battle-start` / `pvp-turn` / `pvp-intent` / `pvp-state-resync` / `pvp-cancel`), server FSM, 10-phase rollout (P-0 through P-10).

**P-1 — server scaffold + wire roundtrip:**
- `pvp-arbiter.js` (root, ESM, Node-clean) — battle FSM module. Exports: `createBattle`, `buildStartFrame`, `endBattle`, `handleIntent`, `handleDisconnect`, `reapStalled`, `getActiveCount`, `getBattleForUser`, `_testReset`.
- Per-battle state shape: `{battleId, turnIdx, status, pendingIntents, combatants, sideA, sideB, rngState}`. `_battles` Map keyed by battleId; `_userBattle` Map keyed by userId for one-battle-per-user enforcement.
- ws-presence handlers: `pvp-arb-start` (test entry — spawn battle, immediately end as draw stub), `pvp-intent` (envelope validation only — kind allowlist, turnIdx match, battle exists). P-4 will land real turn resolution.
- Disconnect hook: if a player drops mid-battle, survivor receives `pvp-cancel reason: 'opponent-disconnect'`. Lives parallel to legacy `_pvpPartners` cleanup until P-10.
- `PVP_ARBITER = false` flag in `src/net.js`. Doc-gated; existing PvP path unchanged.
- Per-kind rate limit `pvp-intent: { cap: 3, refill: 1 }`.
- 5 new wire-sim tests (79 → 84): battle roundtrip, opponent-offline rejection, already-in-battle rejection, stale-turn silent rejection, post-end disconnect no double-fire.

**No production behavior change.** `PVP_ENABLED` is still `false` (see [[ff3mmo-pvp-disabled]]) so the entire PvP path including the new arbiter is unreachable from real gameplay. P-2 through P-8 progressively land stat generation + battle math + AI + viewer + input rewire. P-9 flips both `PVP_ARBITER` and `PVP_ENABLED` together.

**Gates:** lint 0, pvp-wire-sim 84/84.
...
No code changes. Audit of every path where the client asserts inventory state
to the server (trade, give-item, save sync, profile broadcast, cross-device
replay) lands as `docs/INVENTORY-MIRROR-PLAN.md` with a 6-phase rollout plan
for the server-side inventory mirror — the only mechanism that genuinely
closes the dup vectors. Smaller forensic + rate-limit fixes are catalogued
in the plan but explicitly out of scope; they make dup harder to script and
easier to detect but don't prevent it.

Five vectors enumerated:
- **V-A trade dup** — server doesn't validate sender owns item (forensic-only defense via `tradeLog`)
- **V-B give-item dup** — same trust model, no audit log
- **V-C save sync** — biggest hole, client can write arbitrary inventory/gil/jobs within structural limits
- **V-D update broadcast** — equipment IDs accepted without inventory cross-check; inflates co-op AI-ally stats
- **V-E cross-device replay** — V-C via login, naturally closed when V-C closes

Phased mirror plan: Phase 0 (read-only mirror seed) → Phase 1 (authoritative `inv-event` wire) → Phase 2 (server-rolled loot/shop) → Phase 3 (trade-on-server) → Phase 4 (save = mirror snapshot) → Phase 5 (`update` cross-check) → Phase 6 (cleanup). Each phase is independently shippable with a feature-flag rollback path. Estimated 3-6 weeks for full rollout.

**Recommendation:** Tier 1 — document only (this version). Defer Tier 2 fixes
unless dup abuse surfaces in the `trades` audit log. Tier 3 trigger conditions:
observed abuse, PvP re-enable decision (V-D becomes load-bearing for fair PvP),
or launch out of open beta.

Memory: new `project_ff3mmo_dup_vectors.md` indexes the plan + cross-links to
existing trust-model documentation.
...
`makeItemDropText` in `src/text-utils.js` (used by `_drawRewardText` for the battle victory drop screen) called `getItemNameClean`, which strips the leading icon byte from the ROM name + skips the subtype-icon fallback. Result: staff/sword/etc. drops rendered as just "Staff!" with no glyph. Chest treasures already got it right via `map-triggers.js#getItemNameShrines`.

Switched the drop helper to `getItemNameShrines` — same path inventory, shop, and chest rows use, so a staff drop now shows `[staff $EA] Staff!`, a sword shows `[sword $EF] Sword!`, etc. The v1.7.453 subtype-fallback (basic staffs/rods whose ROM names lack a leading icon) gets picked up too.

## 1.7.731 — 2026-05-25

### Fix: gate content lands in the right mode at first paint (follow-up to v1.7.730)

v1.7.730 stopped the gate from flashing the body content, but the gate itself was still rendered in its closed-beta default ("Beta password required." + visible password input) and only rewrote to the open-beta "Tap to enter." after the deferred module finished loading. Users on an open-beta server saw the closed-beta-styled splash briefly before it morphed.

Added two new server-side template tokens emitted by `server.js`:
- `{{GATE_SUB_TEXT}}` → `'Tap to enter.'` (open) or `'Beta password required.'` (closed)
- `{{GATE_INPUT_DISPLAY}}` → `'display:none'` (open) or `''` (closed)

`index.html`'s `#pw-sub` and `#pw-input` consume them so the HTML reaches the browser already in the right mode. The module script still rewrites the same fields as a safety net for static-served pages (no template substitution).

## 1.7.730 — 2026-05-25

### Fix: splash no longer flashes the body content before appearing

On first page load, open-beta visitors saw a couple seconds of the underlying body (h1 "FF3 MMO", mobile controls deck on phones) before the tap-to-enter splash finally covered the page. Root cause: `server.js` was injecting `style="display:none"` on `#pw-gate` for open-beta servers (so it didn't sit visible until the deferred `<script type="module">` got around to wiring its tap handler), then the module ran `gate.style.display = ''` to unhide. The window between HTML parse and module-eval — real seconds on slow mobile, because the module has to fetch its imports — is what flashed.

Inverted: the gate is now ALWAYS visible from first paint (`GATE_DISPLAY` always empty in `server.js`). To avoid an inverse problem (returning-this-tab visitors seeing the splash re-flash on reload before `unlockGate()` runs), a tiny synchronous head script in `index.html` reads `sessionStorage.ff3_auth` and adds `gate-passed` to `<html>` before body parse; CSS rule `html.gate-passed #pw-gate { display: none !important; }` keeps the splash hidden for those visitors. Both branches run pre-body, so neither sees a flash.

Storage-gesture rule unaffected ([[ff3mmo-storage-gesture-rule]]) — fresh visitors and new tabs still get the splash and the first IndexedDB access still happens inside a user-activation context.

## 1.7.729 — 2026-05-25

### Fix: battle menu visibility is hide-list-driven (was a brittle per-state allowlist)

`_battleMenuStates#isMenu` in `src/battle-draw-menu.js` was a giant `bs === 'menu-open' || bs === 'target-select' || ... || bs.startsWith('ally-')` allowlist of ~25 literals. Any new battleState that wasn't patched into that OR-chain silently blanked the bottom action menu mid-battle — the [[ff3mmo-predicate-coverage]] pattern, burned previously at v1.7.231 and v1.7.431. Audit caught at least two long-standing gaps that no one had reported: `fenix-revive` (Phoenix Down sub-FSM) and `pvp-dissolve` (PvP opponent death) both blanked the menu while the rest of the battle UI kept running.

Inverted to a hide-list: `_HIDE_MENU_STATES = {'none','roar-hold','flash-strobe'}` (pre-battle intro states where the menu hasn't appeared yet). `isMenu = !_HIDE_MENU_STATES.has(bs) && !isSlide && !isAppear` — every other in-battle state inherits menu visibility for free. The `isSlide` and `isAppear` excludes preserve the existing slide-in / boss-appear branches that render only the sliding border without menu contents.

No behavior change in already-visible states (every state previously in the allowlist remains visible). What changes: `fenix-revive`, `pvp-dissolve`, and any future FSM state added without an allowlist patch will now keep the menu drawn.
...
Follow-up to v1.7.725. Closes the remaining four enemy-name box bugs surfaced by the audit.

**Bug 2 — xN count decremented before the death animation.** `_battleEnemyNames()` (`src/battle-draw-menu.js`) filtered by `m.hp > 0`. The instant a hit zeroed a monster's hp the name snapped from "Goblin x3" to "Goblin x2" while that third sprite was still mid-death-anim (sometimes 1–2 frames before `dyingMonsterIndices` even populated). Fix: extracted a shared `isMonsterStillVisible(i)` predicate from `battle-draw-encounter.js` that mirrors the sprite-render skip check (`hp > 0` OR `dyingMonsterIndices` mid-`monster-death` OR being-hit/magic-target across the kill-pending states). The name list now decrements in lockstep with sprite disappearance, never ahead of it. Sprite renderer re-uses the same helper for its skip check — single source.

**Bugs 3 & 4 — PVP name list filtered by target.** `drawBattleMenu` gated the main opponent on `pvpPlayerTargetIdx < 0` and the ally loop on `i >= pvpPlayerTargetIdx`. Targeting ally idx 2 hid the main opp + allies 0/1 from the stacked name list. Fix: always push the main opp when alive (`!enemyDefeated` + optional `hp > 0` belt), and always push every living ally regardless of `pvpPlayerTargetIdx`. PVP target tracking is the responsibility of the CURSOR, not the name list. (PVP is off-by-default per [[ff3mmo-pvp-disabled]] but the rewrite path will exercise these gates.)

**Bug 5 — long names spilled into the action-menu column.** The outer save/clip in `drawBattleMenu` is `(8, HUD_BOT_Y, CANVAS_W - 16, HUD_BOT_H)` — full HUD width, shared with the Fight/Magic/Item/Run renderer on the right. A long monster name + " x2" suffix could visually overflow past the name panel's right edge into the menu column. Fix: wrap the name-text block in its own save/clip scoped to `(0, HUD_BOT_Y, boxW=120, boxH=64)`, restore after. Text now hard-clips at the name panel's right edge.
...
1. Removed the `_diagStatus` helper + call site in `src/roster.js` (v1.7.718 temp log that posted to `/api/client-error` for pm2-side grepping of status-sprite receipt). Confirmed working — no longer needed.

2. `/disband` chat message restored to `* You disbanded the party` (was the awkward `* Disband sent to server` from v1.7.720). `/leave` similarly back to `* You left the party`. The unconditional-server-send semantics from v1.7.720 are unchanged — from the user's POV the local state is cleared and the wire fired, so the affirmative wording is accurate.
...
When a party invite is accepted, every party view now hears FF3 NSF track 44 for 5 s and the inviter's "Joined" message box holds for the same duration (was 1 s). Fires at three sites:

- **Inviter side** (`_resolveAsJoin`): plays when `party-invite-result` arrives with `accept: true`.
- **Acceptor side** (`setNetPartyInviteHandler` accept callback): plays when the user taps Z to confirm.
- **Existing-member side** (`setNetPartyMemberJoinedHandler`): plays when the server's `party-member-joined` arrives **with `reason: 'accepted'`**.

The `reason` field is new (server side) — set only by the post-invite-response fanout. The reconnect-time fanout (in `case 'hello'`) deliberately omits it so a partymate's phone-wake-from-pocket doesn't re-jingle every existing-member client every time. Without this gate, mobile-Safari WS suspend cycles would have re-played the jingle dozens of times per session.

`playPartyJoinJingle()` lives in `src/music.js`:

- Guards against concurrent triggers via `_partyJoinTimer` — second call while a jingle is in flight is a no-op (prevents `pauseMusic` from double-stashing and losing the original map music when `resumeMusic` later fires).
- Pauses current map music → `playTrack(44)` → 5 s timer → `resumeMusic()`.
- Early-returns if `nsfData` isn't loaded yet (boot-time safety).

`JOINED_HOLD_MS` in `src/party-invite.js` raised from 1000 → 5000 so the "Joined" message box and the jingle ring out together. The inviter sees "Joined" for 5 s; the acceptor's prompt closes on their tap (no message-box change there — the tap itself is the affirmation).
...
Temporary diagnostic to confirm whether `p.statusMask` is reaching the receiver and whether the sprite map is populated at draw time. `roster.js#_diagStatus(p)` logs one line per (player, mask) transition (deduped, no per-frame spam) to BOTH the local console AND `/api/client-error` — the latter so the line lands in pm2 logs server-side and we can grep for `STATUS-DIAG` instead of needing devtools on a phone.

Format: `[STATUS-DIAG] name=<peer> mask=0x<hex> isReal=<bool> spriteMapSize=<n> hasPoisonFrames=<bool>`

To verify: get one phone poisoned, then on the **receiving** phone, the next time the poisoned peer's row renders, the line fires once. Grep prod with `pm2 logs server | grep STATUS-DIAG`.

Delete `_diagStatus` helper + call site once verified.
...
User reported the status sprite wasn't showing on the roster row. Two
adjustments:

1. **Dropped the `fadeStep < ROSTER_FADE_STEPS` gate.** Renders
   whenever `p.statusMask` is non-zero — no transition / fade
   conditions hide it.
2. **Y offset matches battle convention.** `portraitY - 4` (matches
   `battle-draw-allies.js:264` and `battle-draw-player.js:224`)
   instead of `-3`.

Wire chain (verified end-to-end this session, still landing as
v1.7.715 plumbing):
- `ps.status.mask` flips in `status-effects.js#addStatus`
- main.js profile builder reads `ps.status.mask` → `statusMask`
- net.js 500ms diff poll detects change → sends `update`
- ws-presence.js `_normalizeProfileField` accepts `statusMask`
  (`0..0x3FF`); update whitelist forwards to peers
- net.js `player-update` handler merges into `_onlinePlayers`
- roster.js `_drawRosterRow` calls `drawStatusSpriteAbove` when
  `p.statusMask` is truthy

Files:
- `src/roster.js` — drop `fadeStep` gate; y offset `-3` → `-4`.
...
`UR_VILLAGER_RED` (magenta hair, bundle 0x01DF10) spawned at (11, 28) —
one tile east of `UR_QUEST_NPC` (peach hair, bundle 0x01E210) at
(10, 28). Two adjacent NPCs with similar-tone hair read as a duplicate
cluster.

Moved 2 spawns:
- `UR_VILLAGER_RED`: (11, 28) → (3, 13) — NW corner, far from the
  south plaza
- `UR_HOODED_SAGE`: (16, 24) → (20, 18) — east plaza, away from PEACH
  + QUEST cluster

New 5-NPC distribution:

| Zone | NPC | Spawn |
|---|---|---|
| NW | UR_VILLAGER_RED (magenta) | (3, 13) |
| W  | UR_VILLAGER_MAIDEN (yellow) | (7, 19) |
| S center | UR_VILLAGER_PEACH (peach) | (15, 25) |
| S (static) | UR_QUEST_NPC (peach, adult body) | (10, 28) |
| E plaza | UR_HOODED_SAGE (magenta, hooded) | (20, 18) |

Verified each new spawn passes the wander `openArea` rule (walkable +
≥3 walkable neighbors).

Files:
- `src/data/town-npcs.js` — updated `TOWN_NPCS[114]` rows for
  `ur_hooded_sage` and `ur_villager_red`; comment updates on their
  specs.
...
**Server.** `case 'party-invite'` was only rejecting if the TARGET was
already in a party — never the INVITER. So a member could chain new
parties: Alice → Bob accepts (Bob is Alice's member), Bob → Carol
accepts (Bob is now Carol's inviter AND Alice's member). Cascading
broken topology. Added an inviter-side check that returns the new
`self-busy` reason; client surfaces "Already in a party" instead of
generic `busy`.

**Profile field.** New server-derived `inParty: 0|1` flag in every
profile-shape payload (snapshot, player-join, dedicated
`_broadcastInPartyChange` after every `_partyMemberships.set/.delete`).
Server is the source of truth — clients can't fake it (not in
`_normalizeProfileField`). `_isUserInParty(uid)` checks both member
(`_partyMemberships.has`) and inviter (any value in
`_partyMemberships.values()`).

**Client.** Roster menu now shows a dimmed "InParty" label (NES `$10`
gray) for any target whose `inParty=1` AND who isn't in YOUR party.
Selecting it prints "<Name> in another party" instead of attempting
the invite. Existing "Cancel" / "Dismiss" labels still take precedence
when applicable.

**Defense in depth.** Server's invite-time rejects (`busy` for target,
`self-busy` for inviter) remain so a misbehaving / stale client can't
slip an invite through.

Files:
- `ws-presence.js` — `_isUserInParty(uid)` +
  `_broadcastInPartyChange(uid)` helpers; `inParty` field on every
  profile broadcast (`_snapshotPayload`, `player-join`); inviter check
  in `case 'party-invite'`; broadcast calls in `party-invite-response`
  accept / `party-dismiss` / `party-disband` / `party-leave`.
- `src/party-invite.js` — `setNetPartyResultHandler` handles
  `self-busy`; `cancelPartyInvite('self-busy')` → "Already in a
  party".
- `src/input-handler.js` — `_rosterMenuPartyAction` refuses with
  "<Name> in another party" if `target.inParty` and not in my party.
- `src/roster.js` — `drawRosterMenu` derives `targetInOtherParty`,
  dim-renders the "InParty" label with NES `$10` palette.

## 1.7.710 — 2026-05-25

### Party members pin to top of roster — strangers scroll-bumped first

Followup to v1.7.709. Pre-fix ordering was `[...sameLocReal,
...remoteParty, ...fake, ...fadingOut]` — strangers walking into your
loc landed ABOVE remote partymates in the visible window. With a full
3-member party + 1 stranger at your loc, the stranger took a top slot
and a remote partymate got scroll-bumped off the visible 3-row
window. Felt wrong: random foot traffic shouldn't push your party
out of the default view.

New ordering: `[...allParty, ...strangersAtLoc, ...fake,
...fadingOut]`. Party (online + offline-cached) always pinned to the
top in `partyMembers` insertion order; strangers slot below. Overflow
still uses the existing `rosterScroll` machinery + blink up/down
arrows when `players.length > 3` — but the SCROLLABLE rows are always
strangers, never partymates.

Refactored `_partyRosterEntries` to return ALL party (no same-loc
exclusion at the source), and added `_strangersAtLoc(loc, partyNames)`
to filter the same-loc real list against the party set. Both `seen`
sets collapsed into one `partyNames` per call.

Files:
- `src/roster.js` — `_partyRosterEntries` no-arg, returns all party;
  new `_strangersAtLoc(loc, partyNames)` helper; `getRosterPlayers`
  + `getRosterVisible` both spread party first.

## 1.7.709 — 2026-05-25

### Party members locked into roster regardless of room or online state

Pre-fix the roster only listed real players in the same `loc` as you —
so the moment your partymate walked into the magic shop (different
`loc`), they vanished from your roster panel. Same on disconnect:
their entry dropped until they reconnected.

`getRosterPlayers` / `getRosterVisible` now slot a permanent
party-member section right after the same-loc real players (and before
the fake `PLAYER_POOL`). Each party member resolved by:

1. `getOnlinePlayerByName(name)` — live profile if online.
2. `partyInviteSt.partyMemberProfiles.get(name)` — cached profile if
   offline (set when they were last seen; survives until they
   explicitly `/leave` or `/disband`).

Deduped against the same-loc real list so a partymate at your loc
shows once, not twice. The green `$75` badge (v1.7.708) carries
through so you can tell partymates from same-room non-party players
at a glance.

Pairs with v1.7.707 (server stopped broadcasting `party-member-left`
on disconnect; client `partyInviteSt.partyMembers` stays stable
through pocket-the-phone blips, so the cached profile is reachable
and the partymate's row never disappears).

Files:
- `src/roster.js` — new `_partyRosterEntries(excludeNames)` helper;
  `getRosterPlayers` + `getRosterVisible` both spread it in after the
  same-loc real list; import `getOnlinePlayerByName` from `net.js` +
  `partyInviteSt` from `party-invite.js`.

## 1.7.708 — 2026-05-25

### Green party badge stays on in battle

The roster row's green party badge ($75 glyph, dark-green palette) only
rendered while the roster was visible — and in battle the roster panel
is replaced by `drawBattleAllies` (game-loop.js: `if (battleAllies > 0
&& battleState !== 'none') drawBattleAllies(); else drawRoster();`),
so partymates lost their badge the moment fighting started.

Mirrored the badge draw into `_drawAllyTexts` at the same coord
(`HUD_RIGHT_X + 32 + 8`) and same palette as roster.js. Predicate
`isInParty(ally)` matches by name. Fades through `ally.fadeStep` so
late-joiners + ally-fade-in transitions look right.

Files:
- `src/battle-draw-allies.js` — import `isInParty`; draw `$75` glyph
  in green over each partymate ally row.
...
Left arrow no longer shows on item page 1 (going left from there lands
on the equipment page, which isn't an "item" page). Left arrow now
shows only from item page 2+. Right arrow unchanged (still shows
whenever more pages exist ahead, including the equip→item1 transition
since items live to the right of weapons).

Files:
- `src/battle-draw-menu.js` — left-arrow condition `itemPage > 0` →
  `itemPage >= 2`.
