...
export function parseSaveSlots(data) {
  ...
  return data.map(s => {
    ...
    return { name: new Uint8Array(s.name), level: s.level || 1, exp: s.exp || 0, hp: s.hp != null ? s.hp : null, stats: s.stats || null, inventory: s.inventory || {}, gil: s.gil || 0, proficiency: s.proficiency || {}, jobIdx: s.jobIdx || 0, unlockedJobs: s.unlockedJobs != null ? s.unlockedJobs : 0x01, cp: s.cp || 0 };
  });
}
