...
//
// After applying the A.W. Jackson English IPS patch, tile IDs map:
//   Digits 0-9:     $80-$89
//   Uppercase A-Z:  $8A-$A3
//   Lowercase a-z:  $A4-$BD
//   Ligatures:      $BE-$DF (single tiles rendering 2 letters: "il"=$CD, "li"=$CE, "ll"=$CF, etc.)
//   Item icons:     $E0-$F5 (shield $E0, robe $E1, mail $E2, helm $E3, gauntlet $E4, bracer $E5,
//                            claw $E6, nunchuck $E7, book $E8, rod $E9, staff $EA, hammer $EB,
//                            spear $EC, knife $ED, axe $EE, sword $EF, katana $F0, harp $F1,
//                            bow $F2, arrow $F3, bell $F4, boomerang $F5)
//   Spell icons:    $72-$75 (Summon $72, ?$73, White $74, Black $75)
//   Space:          $FF
//
// String ID ranges:
...
// --- Character encoding (A.W. Jackson English patch) ---
...
// a-z
for (let i = 0; i < 26; i++) CHAR_MAP[0xA4 + i] = String.fromCharCode(97 + i);
// 0-9
...
function _asciiToTileByte(ch) {
  ...
  if (c >= 97 && c <= 122) return 0xA4 + (c - 97);     // a-z (AWJ)
...
export function getItemNameWithIcon(itemId) {
  ...
  if (hasIcon) {
    // AWJ has dedicated per-class icons baked into the ROM string itself —
    // no overrides needed. Just strip padding spaces between icon and first
    // letter and return.
    let i = 1;
...
export function getSpellNameClean(spellId) {
  ...
  const out = [];
  // Allowlist: digits 0x80-0x89, uppercase 0x8A-0xA3, lowercase 0xA4-0xBD,
  // ligatures 0xBE-0xDF, basic punctuation 0xC1/0xC2/0xC4/0xC5/0xC6/0xC7/0xC8,
  // space 0xFF. Drops magic-school / item-class icon tiles and other padding.
