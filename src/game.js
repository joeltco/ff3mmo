...
function _gameLoopDraw() {
  ...
  if (transSt.state === 'loading') {
    if (loadingSt.state === 'in') _tabFade = Math.min(Math.floor(loadingSt.timer / LOAD_FADE_STEP_MS), LOAD_FADE_MAX);
    else if (loadingSt.state === 'visible') _tabFade = ROSTER_FADE_STEPS;
    else if (loadingSt.state === 'out') _tabFade = LOAD_FADE_MAX - Math.min(Math.floor(loadingSt.timer / LOAD_FADE_STEP_MS), LOAD_FADE_MAX);
  }
