...
export const SFX = {
  ...
  POND_DRINK:   0x91,  // SFX $50 + $41 — healing drink (play half).  [UNVERIFIED — never heard; implied write $D0 has not appeared at any site. Explicitly NOT evidence it is wrong: the identical argument was made about WARP in v1.7.874 and WARP measured correct. Needs the right tile found.]
  ...
  RUN_AWAY:     0x74,  // SFX $33 + $41 — escape success (ROM writes $B3 to $7F49 at PC=$BCBC).  [UNVERIFIED — the routine is confirmed, a natural escape is not. The ROM has one `LDA #$B3` (0x67cc9), in a routine at $BC89 that loops 32 times setting a per-character flag ($7D83,X |= $80) across all four party slots and then plays it. Patching the party-hit store to `JMP $BC89` does fire $b3 -> 116 and visibly clears the battlefield, so routine and value are right — but a forced jump is not a player escaping. Two dead ends: that loop's `JSR $8AE6 / AND #$20 / BEQ` is NOT a joypad test ($8AE6 is `INC $B6 / LDA $B6 / RTS`), and Run is a MENU ROW (Attack/Guard/Run/Item), not a held button.]
