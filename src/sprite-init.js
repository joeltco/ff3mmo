...
// Poison bubble animation — 2-frame 8×16 sprite (vertical stack) from ROM $56A50
// Arrangement: frame1 = C(top)+A(bottom), frame2 = D(top)+B(bottom)
// NES sprPal0: $0F=black, $16=dark red, $36=light orange, $30=white
const POISON_PALETTE = [0x0F, 0x16, 0x36, 0x30];
const POISON_FRAME_TILES = [
  [new Uint8Array([0xff,0x7e,0x7c,0xf8,0xf0,0x80,0x00,0x00, 0xfe,0xfc,0xf8,0xf0,0x80,0x00,0x00,0x00]),   // C (top)
   new Uint8Array([0xf0,0xf8,0x7c,0x3e,0x3e,0x3f,0x7f,0x7f, 0x00,0xf0,0xf8,0xfc,0xfc,0xfe,0xfe,0xfe])],  // A (bottom)
  [new Uint8Array([0x07,0x1f,0x3f,0x7e,0x7e,0xfe,0xfe,0xfe, 0x00,0x07,0x1f,0x3f,0x3f,0x7f,0x7f,0x7f]),   // D (top)
   new Uint8Array([0xff,0x7e,0x3e,0x1f,0x07,0x0f,0x1f,0x18, 0x7f,0x3f,0x1f,0x07,0x03,0x07,0x08,0x00])],  // B (bottom)
];

export function initPoisonBubble() {
  return POISON_FRAME_TILES.map(frameTiles => {
    const sc = document.createElement('canvas');
    sc.width = 8; sc.height = 16;
    const sctx = sc.getContext('2d');
    for (let t = 0; t < 2; t++) {
      _blitTile(sctx, decodeTile(frameTiles[t], 0), POISON_PALETTE, 0, t * 8);
    }
    return sc;
  });
}
