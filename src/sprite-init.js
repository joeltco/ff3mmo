...
function _legacyInitBattleSpriteForJobInline(romData, jobIdx) {
  ...
  // For Monk (jobIdx 2), use PPU-captured Monk tiles so the L-fwd unarmed strike
  // pose actually exists (the ROM-banking generic path can't reach these tiles).
  if (jobIdx === 2) {
    const d = (raw) => decodeTile(raw, 0);
    ...
    const atkRBackTiles = [idleTiles[0], idleTiles[1], d(MO_R_BACK_T2), idleTiles[3]];
    const atkLBackTiles = [idleTiles[0], d(MO_L_BACK_T1), idleTiles[2], d(MO_L_BACK_T3)];
    const atkLFwdTiles  = [idleTiles[0], idleTiles[1], d(MO_L_FWD_T2), d(MO_L_FWD_T3)];
    const battleSpriteAttackCanvas   = _renderPortrait(atkRBackTiles, _BATTLE_LAYOUT, palette);
    const battleSpriteAttackLCanvas  = _renderPortrait(atkLBackTiles, _BATTLE_LAYOUT, palette);
    const battleSpriteAttack2Canvas  = _renderPortrait(idleTiles, _BATTLE_LAYOUT, palette);
    const battleSpriteAttackL2Canvas = _renderPortrait(atkLFwdTiles, _BATTLE_LAYOUT, palette);

    const battleSpriteKnifeRCanvas    = _renderPortrait(atkRBackTiles, _BATTLE_LAYOUT, palette);
    const battleSpriteKnifeLCanvas    = _renderPortrait(atkLBackTiles, _BATTLE_LAYOUT, palette);
    const battleSpriteKnifeBackCanvas = _renderPortrait(atkLBackTiles, _BATTLE_LAYOUT, palette);
    initWeaponSprites(palette);
...
function _initFakePosePortraits(romData) {
  ...
  const fakePlayerAttackLPortraits  = _genPosePortraits([idleTiles[0], decodeTile(_FP_KNIFE_L[1], 0), idleTiles[2], decodeTile(_FP_KNIFE_L[3], 0)]);
