...
// Poison sparkle animation — 2-frame 16×8 sprite (2 tiles side by side) from ROM $1B380-$1B3B0
// NES sprPal0: $0F=black, $36=pink — sparkles only use palette index 1
const POISON_PALETTE = [0x0F, 0x36, 0x30, 0x16];
const POISON_FRAME_TILES = [
  [new Uint8Array([0x00,0x02,0x05,0x02,0x00,0x20,0x50,0x20, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00]),  // F1 left ($1B380)
   new Uint8Array([0x00,0x00,0x0c,0x12,0x12,0x0c,0x00,0x00, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00])], // F1 right ($1B390)
  [new Uint8Array([0x00,0x00,0x02,0x05,0x22,0x50,0x20,0x00, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00]),  // F2 left ($1B3A0)
   new Uint8Array([0x00,0x0c,0x12,0x12,0x0c,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00])], // F2 right ($1B3B0)
];
...
export function initPoisonBubble() {
  return POISON_FRAME_TILES.map(frameTiles => {
    ...
    sc.width = 16; sc.height = 8;
    const sctx = sc.getContext('2d');
    _blitTile(sctx, decodeTile(frameTiles[0], 0), POISON_PALETTE, 0, 0);  // left
    _blitTile(sctx, decodeTile(frameTiles[1], 0), POISON_PALETTE, 8, 0);  // right
    return sc;
  });
}
