...
function _generateFloor(romData, floorIndex, seed) {
    ...
    if (floorIndex === 0) {
      ...
      // south-half columns (the half with the exit). v1.7.652.
      {
        ...
        if (southCols.size > 0) {
          ...
          if (doorPos) {
            // Locked room is now a SEPARATE map (mapId 1010) — no in-map
  ...
  // Chamber door → standalone locked-room map (mapId 1010). Engine's
  ...
  // dungeonDestinations entry, which uses `{ goBack: true }` to pop
  // back to the chamber map at the saved position. v1.7.665.
  if (lockedRoomChamberDoor) {
    const chTrig = triggerMap.get(`${lockedRoomChamberDoor.x},${lockedRoomChamberDoor.y}`);
    if (chTrig) dungeonDestinations.set(chTrig.trigId, { mapId: 1010 });
  }
