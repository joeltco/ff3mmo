...
function _generateFloor(romData, floorIndex, seed, dungeon = STARTING_DUNGEON) {
  ...
  // Destinations a floor branch knows by COORDINATE rather than by trigId.
  // ⛔ A trigId IS NOT STABLE. `processTriggerTiles` numbers each type in scan
  // order, so a floor that gains one more type-1 tile renumbers every type-1
  // trigger after it. `spine` wired its boss door as `1:0` and its staircase as
  // `1:1` on exactly that assumption, and adding a locked door to it would have
  // handed `goBack` to the boss door and pointed the entrance staircase at the
  // boss room — step off the stairs and back on and you skip the floor. Same
  // failure the note at the bottom of this file records from the other side.
  // Resolved against the freshly-built triggerMap below.
  const coordDestinations = new Map();
  ...
  if (isBossFloor(dungeon, floorIndex)) {
    ...
    // $01 $49 $01 (wall, passage_btm, wall)
    {
      ...
      var bossDoorX = doorX, bossDoorY = doorY;       // wired by coord below
    ...
    // ⛔ BY COORDINATE, NOT BY TRIG ID. These were `1:0` and `1:1`, resting on
    // "door scans before stairs" — true only while this floor had exactly one
    // door. It now has a second one on some seeds (the locked chamber door), and
    // whichever scans first takes trigId 0.
    coordDestinations.set(`${bossDoorX},${bossDoorY}`, { mapId: bossFloorMapId(dungeon) });
    coordDestinations.set(`${entranceX},${entranceY}`, { goBack: true });
    ...
    // ── A LOCKED DOOR ON THE CENTRE CHAMBER'S WALL ──────────────────────
    //
    // Joel, 2026-08-27: *"add a secret door chance on the wall of the north
    // center chamber"* — and then, when I built it as a carved room sealed by a
    // `secretWalls` tile: *"it shouldn't be drawn, we have a secret door system.
    // why aren't you using it? DOORS THAT REQUIRE FUCKING KEYS."*
    //
    // ⛔ THAT IS THE WHOLE POINT AND I MISSED IT. Dungeon floors set
    // `skipRoomClip`, so anything carved on this map is DRAWN — a hidden room
    // cut into the floor is visible from the moment you walk in, which makes it
    // a room you can see and cannot reach rather than a secret. The game already
    // has the answer and has shipped it since v1.7.649: a chamber door ($70)
    // wired to a STANDALONE MAP. Nothing is drawn on this floor; the room does
    // not exist here at all. `lockedDoors` blocks it with "Locked." until the
    // player has the Magic Key ($98), and the unlock persists through
    // `consumedTiles`.
    //
    // Same three calls Altar Cave's floors 0 and 2 use — `placeChamberDoor`,
    // `lockedRoomDoors`, `lockedDoors`. The only new thing is WHERE it looks:
    // the centre chamber's own wall, rather than anywhere on the map.
    //
    // ⛔ NO ROOM DECLARED, NO DOOR. `lockedRoomMapIdForFloor` returns null for a
    // dungeon that declares none, and a door onto null is a door that opens on
    ...
    if (SPN.lockedDoor > 0 && rng() < SPN.lockedDoor) {
      ...
      if (cands.length > 0 && _lockedId3 !== null) {
        const dPos = cands[Math.floor(rng() * cands.length)];
        placeChamberDoor(tilemap, dPos.x, dPos.y);
        lockedRoomDoors.set(`${dPos.x},${dPos.y}`, { mapId: _lockedId3 });
        lockedDoors.add(`${dPos.x},${dPos.y}`);
      }
    }
  ...
  // Coordinate-keyed destinations a branch registered — resolved to trigIds now
  // that the triggerMap exists, and BEFORE the fallback below so it sees them.
  for (const [coord, dest] of coordDestinations) {
