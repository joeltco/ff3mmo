...
- **Damage spells (Black Mage)** — only Cure + Poisona shipped (1.6.77). INT-based formula and `magic-cast`/`magic-hit` pipeline are already there; needs spell content (Fire/Blizzard/Bolt etc.), per-spell SFX, and per-spell anim sprites. Cure animation now covers the entire white-magic school (`src/cure-anim.js`, 1.7.10–1.7.20, widened to status-cure + revive in 1.7.23 after Poisona REC confirmed shared tiles); damage spells still need their own captures.
- **Per-spell anim + SFX** — White-magic cast animation (build-up flame + 8-star ring + heal-phase tile flicker) and cast SFX (`SFX.MAGIC_CAST = 0x62`, FF3J disasm 33/B0FF) cover all white-magic spells: recovery (Cure family), status-cure (Poisona, Bndna), and revive (Raise) all route through `cure-anim.js` via `_isCureAnimSpell()` in `spell-cast.js`. Damage spells still fall through to legacy 1100 ms timing with `SFX.SW_HIT` as a placeholder hit sound (`spell-cast.js:105`). Each remaining damage spell needs its own per-element captured tile set + hit SFX entry.
...
- **Generic job death pose** — `buildDeathPoseCanvases(bundle)` in `combatant-sprites.js:323` returns `null` when `bundle.death` is missing (true for jobs 3–21, no PPU capture yet). Caller in `sprite-init.js:1021` falls back to `bodies.idle` — but that's the *opponent* body which is pre-h-flipped. For roster ally death pose render (`battle-drawing.js:1326`) the canvas is drawn directly without a counter-flip, so a defeated WM ally shows up as a mirrored idle pose. Fix paths: (a) capture per-job death tiles via REC OAM and feed `bundle.death`, or (b) change the fallback in the bundle path to use a non-flipped idle source (the ally idle full-body) so at least the orientation reads correctly until real death tiles land. (b) is the cheaper interim; (a) is the canonical fix.
...
- **Per-hit cycle.** Each hit goes through three states: `attack-back` (wind-up pose) → `attack-fwd` (transition, `FWD_SWING_MS`) → `player-slash` (impact, `getSlashHoldMs(weaponId)` from `slash-effects.js` — per-weapon: blade `3 × 30 ms = 90 ms`, impact `2 × 30 ms = 60 ms`). On a miss, `player-slash` short-circuits (gated by `shouldDrawSlash(hit)`) so there's no dead time when the slash overlay won't render. Same predicate gates `ally-slash` and `pvp-enemy-slash`.
