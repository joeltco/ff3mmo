...
- **Damage spells (Black Mage)** — only Cure + Poisona shipped (1.6.77). INT-based formula and `magic-cast`/`magic-hit` pipeline are already there; needs spell content (Fire/Blizzard/Bolt etc.), per-spell SFX, and per-spell anim sprites. Each new spell drops a single entry into `SPELL_ANIM_DATA` in `src/spell-anim.js` (1.7.49) with captured tile bytes + per-phase render functions; render sites dispatch automatically by spell ID.
- **Per-spell anim + SFX** — Per-spell animations live in `src/spell-anim.js` as a registry keyed by spell ID. Cure (`0x34`) and Poisona (`0x35`) are the first two entries — distinct tile bytes and palettes per spell, NOT a shared "flame + palette swap" (the prior cure-anim.js model was wrong: REC OAM 2026-05-06 confirmed Cure and Poisona use entirely different tile data). Cast SFX is `SFX.MAGIC_CAST = 0x62` (fired for every spell at cast start, `spell-cast.js:72`); Cure and Poisona share `SFX.CURE` at the heal moment per the captured `$7F49 = $40` queue value. Damage spells still fall through to legacy 1100 ms timing with `SFX.SW_HIT` as a placeholder hit sound (`spell-cast.js:105`). Adding a damage spell anim: capture 50 frames via REC OAM, drop tile bytes + phase rule into the registry, define a hit-SFX entry — no render-site changes.
