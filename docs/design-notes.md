...
- ~~**Rod weapon sprite**~~ — DONE in v1.7.821. Captured from the PPU along with axe / spear / katana / hammer / book / bell / harp. Tooling + method: `tools/monscan/README.md`.
- **Bow and arrow have no HELD overlay, by design** — investigated and closed. The game draws no held bow sprite: 87 unique bow-only poses across the whole screen contain nothing bow-shaped. The bow lives in the character's own attack pose, the way `claw` does, so `combatant-pose.js` returning null for both is correct, not a gap. What the earlier 12-14px capture actually showed was the arrow in flight, which now ships as a projectile (v1.7.823-824).
- **Six monsters have guessed palettes** — 0x35 Fury, 0x63 Captain, 0x66 Phoenix, 0x8D Hobgoblin, 0xBE Spriggan, 0xBF TerribleD. They appear in no encounter monster list in EITHER ROM, and the 16-byte property block carries no palette field (best-correlating byte matches 5 of 452). So no palette exists in the ROM to capture — the search there is closed. Artwork is correct (inherited via shared RAW constants); only colors are a guess. Recoverable from a reference image via `tools/monscan/palette-from-image.py`. Low value: none of the six can ever spawn.
- **Bow mechanics were specified, not measured** — arrows consumed per shot, bow needs arrows equipped, one in each hand. The ROM probe intended to confirm this failed (documented in the v1.7.822 entry), so the behaviour matches the design decision rather than verified FF3 canon.
- **`twoHanded` is never enforced** — set on 8 items (all bow and harp subtypes) and read by nothing. Bows deliberately pair with arrows in the other hand, so enforcing it would need a rule that exempts the quiver.
...
- ⛔ **A secret corridor must open into the ROOM, not into a one-tile nook.** `findCorridorCandidates` checked only the single tile behind the wall. On floor 0 seed 1799000372193 that tile was FLOOR and the one beyond it was CEILING, so the corridor connected to an isolated pocket and its whole 5-tile run was sealed — one tile over `sealTinyPockets`' 4-tile cap, so it survived to the sweep as "5 sealed pocket tiles". The scan now requires the tile TWO in to be walkable as well. Latent for as long as the corridor existed; v1.10.31's room sampling started landing on it.
- ⛔ **Floor 0's ceiling snake is gated** (`check-floor-plan.mjs`, v1.10.37): all seeds must produce ONE ceiling perimeter. A split snake breaks no correctness invariant — floors stay connected, exits reachable, chests openable — so nothing else can see it. `roughenOverhang` (v1.10.34) cut it on 197 of 200 seeds for three releases; floor 0 is excluded from that pass now, because its band IS the cave outline rather than a slab lid.
- ⛔ **Floor 0's two halves must not overlap, and the neck splits at the GAP, not column 16.** v1.10.31 sampled each half-width independently and 5 of 12 combinations overlapped; when the left half reached column 16 the neck's right-hand scan found room A's own tiles and joined nothing, leaving room B a separate cave with its own arch and chests. The half-width is derived from a rolled gap now.
...
- ⛔ **THE TILE GRAMMAR COMES FROM THE ROM, NOT FROM COMMENTS.** `tools/tile-grammar.mjs` censuses every ordered vertical and horizontal tile pair across ROM cave maps 111, 112, 113, 22 and 115, and fails on any arrangement we produce that they never contain. It exists because a wall arrangement got invented and shipped — a ceiling-looking tile with rock above it — while the rules being checked were ones written in comments.
  - ⛔ **Sample size decides what is a law.** The five ROM caves hold CEIL 2664, VOID 1712, FLOOR 348, ROCK 326 — then CHEST 16, BONES 12, WATER 7, DOOR 1. "The cartridge never puts rock above a door" is not a rule when the cartridge has ONE door. Only the four structural tiles are treated as grammar; the rest is reported as insufficient evidence.
  - ⛔ **Judge by what a tile LOOKS like, not its id.** `$44` FALSE_CEILING draws identically to `$00` CEILING, and the ROM caves contain no `$44` at all — so a pair census keyed on ids excused every arrangement involving it, which is exactly where the shipped bug lived.
  - **What it found:** FLOOR touching VOID, 281 tiles per 120 floor-0 generations, mostly at the entrance frame — `placeEntrance` lays a 3-wide landing and the outer tiles hang over black where the cave beneath is narrower. The cartridge always walls floor: below it FLOOR or CEIL, above it FLOOR or ROCK, beside it FLOOR/CEIL/ROCK, never VOID. `sealFloorToVoid` applies exactly that.
- ⛔ **A CEILING CAPPING A ROCK BAND HAS EXACTLY TWO ROCKY TILES BELOW IT.** Not one, not three. Measured in the cartridge's own caves — ROM maps 111, 113, 22, 115 — where it is absolute: **125 of 125 bands are depth 2**. `check-floor-shape.mjs` re-derives that from the ROM every run and gates our floors against it.
  - v1.10.34's `roughenOverhang` deepened the band to make it look less like a straight lid and shipped **652 / 831 / 1376 three-deep bands** on floors 1/2/3. Reported from play as "ceiling has 3 wall tiles underneath — that's a rule break". Removed in v1.10.39.
  - ⛔ **The ROM's band looks irregular because its FLOOR EDGES are jagged, not because the band varies in depth.** Contour irregularity has to come from the room and corridor outlines. Deepening the band is not a cheaper route to it — it moves the flatness metric while breaking the rule that metric was a proxy for, which is why band flatness is now REPORTED rather than gated.
  - A residual of a few deep bands per hundred floors survives from a different, unidentified cause (bands stacking where a corridor's wall sits above a room's). The gate's limits sit just above it, so the systematic case cannot return while the rare one stays visible.
...
- ⛔ **`buildCaveShape` is GONE (v1.10.22).** This entry used to document its `clamp` / `maxWidth` parameters as though it were live code. Nothing called it, and nothing called `generateCaveOutline` except it — 155 lines of it, documented as current. Floor 0's outline comes from `generateCaveOutlinePath`, which is a different function. `carvePathwayRoom` and `findInteriorFloor` went with them. `FLOOR_CONFIG[0]` = chests [2,4], skeletons [6,10], secrets 1.
- **DUNGEON debug tab** (`src/debug/tabs/dungeon.js`, v1.10.38) — the generator's output in the Konami panel: floor + seed controls, the floor painted with real ROM tiles, rings for entrance/exits/chests/secrets/warp, unreachable tiles filled red, and the plan printed underneath. Phone-first (≥40px controls, full-width square canvas, scrolling body); the panel header wraps now, since eleven tabs overflow a phone with no scrollbar. ⛔ Gated by `check-debug-dungeon.mjs`, which mounts it against a REAL canvas — nothing else in the suite executes debug-panel code, and a check that only greps the report text passes a broken entrance reference.
- **Phase 1 vocabulary (v1.10.22)** — `src/dungeon/` holds the leaf modules the generator is being taken apart into. `tiles.js` is the single source for cave tile ids, replacing a hand-mirrored copy in `dungeon-locked-room.js` that used a second set of names (`CEILING_TILE`, `ROCK_TILE`, …). `chambers.js` has `carveChamber`, which replaces the room carve that was written out four times.
  - ⛔ **`WATER_EDGE` was two constants with the same name and different values** — `0x08` at module scope and `0x23` redefined inside floor 3's branch, shadowing it for every line below. They are `WATER_EDGE_POND` and `WATER_EDGE_N` now.
  - `corridors.js` (v1.10.23) holds `carveHRun` / `carveVRun` / `carveFatteningVRun` / `carveFatteningHRun` / `carveBand`, replacing **nine inline corridor carves**. Zero 3-row carve loops remain in `dungeon-generator.js`.
  - ⛔ **The horizontal/vertical asymmetry is load-bearing.** A horizontal corridor is carved THREE ROWS tall and `addOverhang` eats the top two into rock, because rock hangs *below* a ceiling lip so a sideways passage needs headroom above it. A vertical corridor is simply 1–2 columns. A `carveLine` treating both axes alike produces floating rock or pinched ceiling and `addOverhang` fights it.
  - ⛔ **Corridor bounds are NOT uniform and must not be tidied.** The inline copies clamped rows to `0..31` in some places and `1..30` in others (floor 3's narrow paths and branch spine use the tighter range), and `carveVerticalPathway`'s yMax is 28 against 29 for the floor-1/2 corridors. Both conventions are parameters; unifying them moves tiles at the map edge.
  - `boss-chamber.js` (v1.10.24) holds the ONE boss-chamber shape plus per-dungeon skins (`CRYSTAL_SKIN`, `CAVE_SKIN`). ⛔ **The crystal pedestal is NOT in the shape** — `$3a`–`$3f` depict an altar only in tileset 2, and rendered with cave assets they are a flat yellow slab (seen, via `floor-png.mjs --boss crystal`). It lives in `CRYSTAL_SKIN.decorate` and is stamped over the shape, so Altar Cave is unchanged. The warp tile `$61` DOES stay in the shape: every dungeon's boss chamber has a way out.
  - `shape.js` (v1.10.25) holds the seven cave-shaping passes and `finishCaveShape`, the five-pass chain that was written out four times. ⛔ **Order is the contract** — each pass assumes the previous ran, and `addOverhang` must be last because it reads the finished ceiling to decide where rock hangs. `openEntranceLanding` and `sealTinyPockets` are deliberately NOT in the chain: the first must run AFTER `addOverhang` or the pocket is re-walled, the second runs at the very end of `generateFloor` after the trap swap.
  - ⛔ **Floor 0 does NOT run the same passes**, and its comment claimed it did ("exact passes/order as every other floor"). It runs three of the five, skipping the pinch fix and protrusion removal, because its shape is one traced ceiling snake rather than carved rooms joined by corridors. Do not "fix" floor 0 into calling `finishCaveShape`.
  - `plan.js` (v1.10.27–28) records a floor's chambers and links AS THEY ARE CARVED. ⛔ **Not build-then-render** — that reorders the rng draws (sizing draws are interleaved with jitter draws) and changes every floor. `node tools/floor-view.mjs <floor> <seed> 1 --plan` prints it. Floors 1/2/3 record every chamber; floor 0 (a traced ceiling snake, not a room list) and floor 4 (authored) are marked `complete: false`, and `check-floor-plan.mjs` pins which is which.
  - ⛔ **RNG call order is part of a carve's contract.** One floor draws from one seeded stream, so a carve making one more or one fewer `rng()` call shifts every later chest, skeleton and corridor. It does not read as a carve bug; the whole floor changes.
- **`tools/check-floor-snapshot.mjs` is a deploy gate.** It hashes every generated map — **wiring as well as tiles** (`triggerMap`, `dungeonDestinations`, `falseWalls`, `lockedDoors`, `rockSwitch`, `warpTile`) — against `docs/FLOOR-SNAPSHOT.json`. Hashing tiles alone would pass a refactor that keeps every tile and breaks the way out; proven on both a tile change and a wiring-only change. Re-baseline with `--update` only for an intentional change, and say so in the changelog.
- **A chest tile is NOT walkable.** `isPassable` opens trigger types 1 and 4 (entrance / door / passage); a chest is type 2, and `movement.js` reaches it via `facedTile === 0x7C`. So you stand beside a chest and face it — which has two consequences that cost real time to relearn:
  - a reachability flood must BLOCK on chests, or a pocket sealed by one reads as open. Floor 3's branch-alcove chest is placed ON the dead-end tile, so anything hanging off that end (a fat stretch, a leftover carve row) had the chest as its only non-solid neighbour — 24 tiles across 23 of 150 seeds;
  - a chest-openability check must NOT ask "is the chest tile reachable" — it never is. Openable = some orthogonal neighbour is reachable. Asking the wrong question called 300/300 locked-room chests broken.
- **`sealTinyPockets` (end of `generateFloor`) fills 1-4 tile islands of floor that nothing can reach.** Runs last, after the hidden-trap swap, so it walks the map the player actually gets. A pocket is filled only if EVERY neighbour is solid under both passability models (`CEILING` / `WALL_ROCKY` / `CHEST` / `FILL_VOID`) — `dungeon-sweep.mjs`'s `PASS` set is deliberately stricter than the game's `isPassable`, so a generic "fill what the flood missed" pass would delete walkable content. Guards: never the entrance's own region, never rows ≥ 22 (the secret teleport room is an intentional island), never a pocket carrying a trigger or any tile that isn't `FLOOR`/`BONES`, never more than 4 tiles (floor 2's puzzle room is ~21 and sealed on purpose). Fill material follows the overhang rule — rocky if the tile above is ceiling/rocky, else ceiling, applied top-down.
  - ⛔ **Do not break the component flood early once it passes `maxSize`.** Queued tiles stop expanding, whatever sits behind them is never marked seen, and the outer scan re-walks that remainder as a fresh "small pocket" and fills it. Measured: an early break took 6 floor-2 seeds down a reachable tile each and cost one seed a whole chest. Caught only because the change was diffed per-seed against the unpatched generator.
- **Floor 3's own "BFS seal unreachable floor" pass floods THROUGH chests on purpose** — it drives the chest DELETION that follows it, and since a chest tile can never be entered, a chest-blocking flood there would mark every chest unreachable and wipe all of them. That is why it cannot catch a pocket sealed by a chest, and why `sealTinyPockets` exists alongside it. Not duplicates: that one deletes unreachable content in bulk, this one fills tiny decorative holes and never removes anything.
- **`tools/dungeon-sweep.mjs` is a deploy gate as of v1.10.15** (`run_gate 60 node tools/dungeon-sweep.mjs 400`, ~1.5s). Altar Cave reseeds on every entry, so a layout bug ships as "some players get a broken floor", never as a reproducible one. Hard failures: unreachable exit staircase, chest with no reachable neighbour, sealed pocket, or floor 2's rock-switch room not opening fully. It also covers the three generated maps that are not floors — the secret teleport room (both `goLeft` variants) and locked rooms 1010/1011, which had no gate of any kind before.
- **Floor 2's sealed puzzle room is now checked, not excused.** The sweep pulls the switch via `applyRockSwitch` and asserts the room opens completely (stranded 0, every chest openable) on every seed. Apply `wt.newTile` exactly as `handleRockPuzzle` does — the list mixes `WALL_ROCKY` with `FLOOR`, so a blanket floor fill models the switch as opening more than it does. Before this, the floor just printed "150/150 seeds strand 3187 tiles" as a soft count next to a comment asserting it was the puzzle; nothing tested the claim.
...
- **⛔ THREE SPELLS AUTO-TARGET EVERY ENEMY, AND THE ROM SAYS SO (v1.10.84).** `spellHitsAllEnemies(id)` → **Meteor 0x02, Quake 0x07, Raze 0x16**. Picking one skips target select entirely and commits with `targetMode: 'all'`; `input-handler.js#_battleInputMagicSelect` routes them down the same `item-list-out` menu-close path the picker itself uses.
  ...
  **Proven causally on the cartridge, both directions** (`tools/monscan/spell-target-probe.cjs`): Fire opens a cursor on one goblin and damages one of four. Patch its byte +5 from `0x08` to `0x48` — one bit, nothing else — and the same spell stops asking and damages all four. Clear bit 6 on Quake and it starts asking again. Then all 56 castable spells were swept on the cartridge: **56/56** agree with "bit 6 set, or a summon". Byte **+7** was decoded in the same pass — it is the **cast halo**, its three groups are exactly black / white / call (they map 1:1 onto the CHR block each cast loads in `spell-sweep.json`), and it agrees with `getSpellSchool` on all 56.

  **Damage numbers.** On the cartridge an all-target cast drops every body's HP within **one frame** (measured: 662/662/663/663 across four goblins). Meteor/Quake/Raze already take ff3mmo's *parallel* apply path, so all four numbers pop together in 0.75 s — it matches with no change needed. The **serial impact walk** (1400 ms per target, one number at a time, ~5 s for four bodies) is what the player-driven all/column picker uses, and that is this game's own presentation for its own feature — the cartridge has no such picker. Left alone deliberately.

  **⛔ SUMMONS ARE AN INTENTIONAL DIVERGENCE.** On the cartridge all eight also skip target select. Here a summon's reach belongs to the TIER system — a Conjurer's roll can land on a single-target attack, a Summoner's third effect always hits everyone (`summon-tiers.js`, `resolveSummonEffect`). `spellHitsAllEnemies` excludes them on purpose so it cannot overrule that. Gate section 5 pins the exclusion so it stays a decision.
...
- **⛔ SUMMON REACH IS THE TIER EFFECT'S `all` FLAG, AND IT WIDENS (v1.10.87).** `startSpellCast` used to read `_summonEffect.all` **only to narrow** (`else if (!all)`). The magic menu commits a summon with `targetMode: 'single'` — summons are excluded from the all/column picker on purpose — so `_targets` arrived holding one enemy and an `all: true` effect had nothing to widen. Measured through the real menu: **Diamond Dust, Tidal Wave, Mega Flair and Zantetsuken all resolved to TARGETS=1.** Zantetsuken instant-KO'd one body instead of the formation. It now widens through `_enemySideTargets('all')` — the same enumeration the picker uses, not a second copy.
  ...
  **The tier table is MEASURED now, not sourced.** Its header said "sourced, not measured off the ROM"; the cartridge agrees with it on every point tested. A summon never opens a target cursor, and **the game labels the target itself**: the box reads **"Everyone"** for an all-target effect and the enemy's own **name** for a single one. An Evoker's Shiva alternates across RNG cursors — Mesmerize → "Everyone", 0 damaged; Icy Stare → "Goblin", exactly 1 of 4 damaged. A Sage's always reads "Everyone" and drops all four bodies' HP **within one frame**.

  ⚠ **Heal/buff effects are a KNOWN, DELIBERATE GAP.** Every `kind: 'heal'` / `'buff'` effect in the table is flagged `all: true`, and they still target the player alone. Honouring `all` there would divide the rolled amount across the party (Ifrit's Healing Light 90 → ~22 each), which is a balance decision rather than a bug fix. Left for a deliberate call.

  ⚠ **`JOB_SUMMONER = 17` is CORRECT and must not be "fixed" to 19.** Those constants index ff3mmo's own `JOBS` array. On the cartridge, ROM job bytes 15/19/20 cast summons, 17 casts black and 18 white — so the ROM byte order and ff3mmo's table agree at 15/18/20 and **disagree at 17/19**. Setting the constant to 19 would hand the summoner tier to ff3mmo's Magus. Nothing reads `JOB_SUMMONER`/`JOB_SAGE` anyway — `summonTierForJob` tests `JOB_CONJURER` alone, and 15 is the Evoker in both orderings. The 17/19 divergence deserves its own look, since `jobs.js` claims "ROM order" and pulls alignment/lvReq from `$72010` by index.

  ⭐ **FF3's RNG is a TABLE, not a tick** — `$FBEF`: `LDX $21 / LDA $15,X / INC $15,X / TAX / LDA $7BE3,X`. Idling frames does NOT move it, so repeated runs of the same boot are **one sample**. Poke the cursor at `$15,X` to vary a roll, and keep an RNG witness (party HP after the round) to prove it moved.

- **⛔ A SUMMON'S EFFECT IS MAPPED INTO THE BATTLE VIEW, ITS CREATURE IS NOT (v1.10.88).** `summon-anim.js#buildFrames` now takes an x scale, and the three parts use two different ones:
  - **creature + cast burst → `sxBand` (1:1 across the full 256 band).** The creature takes over the PARTY PANEL at x 144-255; squeezing it into the view would drag it off the panel it exists to occupy. The cast burst is anchored on the caster (v1.10.86) and is not a view-wide sweep.
  - **effect (and Titan's borrowed Quake crack) → `sxView` (`SCREEN_MAP_W/256` = 144/256).** A summon's effect is the same kind of thing as Meteo's sweep — a full-screen band captured off the NES — so it gets the mapping the screen-anchored spells already use.

  `spell-anim.js` had spelled out why 1:1 is wrong for those back in v1.7.846: *"the map/battle HUD view is only the LEFT 144 px; x 144..256 is the player roster box. A 256-wide band drawn at x=0 therefore spilled 112 px straight across the roster."* Summon effects were still doing exactly that — measured: **Shiva's ice reached x255, Leviathan's Tidal Wave column x253 with its bulk on the roster, and Titan's borrowed Quake crack sat at x160-207, INSIDE the roster box**, while Quake itself is pinned to straddle x=144. `SCREEN_MAP_W` is exported from `spell-anim.js` so there is one definition of "the battle view's width", not two.

  ⚠ **Titan still does not inherit Quake's `SCREEN_PLACEMENT` pin.** Quake straddles the x=144 boundary by explicit request; Titan borrows the same art and now lands at x90-120, in the field. Same crack, two placements — worth a look, not changed here.

  Gate: `check-summon-cast` section 2b pins the containment and that the creature/cast keep the 1:1 mapping.
...
- **`src/data/loot-pools.js`** — Node-clean source of `LOOT_POOLS` + `rollLootEntry(mapId, rng)` / `rollVaseLoot(mapId, rng)` (RNG-injectable). `src/map-triggers.js` imports the table since v1.7.792 (dedupe complete; client retains its own `rollLootEntry` resolver because it returns the raw `{gil:[min,max]}` tuple — caller rolls the amount with the same RNG it uses elsewhere).
...
**Post-arc audit (v1.7.787–794):** see `docs/PVE-REWRITE-PLAN.md#post-arc-audit-v1-7-787--v1-7-794` and `[[ff3mmo-audit-790-794]]` for the full list. The most user-visible additions:
- **Town chest replay block** (v1.7.787) via the `consumed_tiles` SQLite table — `ws-presence.js#case 'chest-open'` marks the row on accept; the validator rejects within 24h. Dungeon chests skip the gate (v1.7.789) — caves regenerate per re-entry so a coord-keyed lookup false-blocks legit chests at recurring coords. Vase mirrors the dungeon-skip since v1.7.790.
- **`pve-encounter-request` gates `zoneKey` against `entry.loc`** (v1.7.794, `_LOC_ZONE_ALLOWLIST` in `ws-presence.js`). Pre-fix a cheater could claim `altar_cave_f4` from Ur or `altar_cave_boss` from anywhere; now rejects with `wrong-zone`. Cheater must also fake `loc` (visible on peers' rosters).
- **`recordIntent` bounds `turnIdx` to `[0, 999]`** (v1.7.791). `endPveBattle` no longer accepts client-overwrite of `battle.intents` (v1.7.792); server's per-turn log is the sole forensics record.
- **Per-kind rate caps** (v1.7.793) for `chest-open` / `vase-search` / `shop-transaction` / `pve-encounter-request` / `pve-battle-end` / `trade-offer`. Bounds the v1.7.789-accepted dungeon-chest replay from ~16,500 gil/s → ~1,100 gil/s in Altar Cave F4.
