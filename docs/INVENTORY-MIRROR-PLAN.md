...
**Status:** SHIPPED + LIVE v1.7.740-746. All 5 original dup vectors closed — though **V-A was only partially closed by v1.7.745** (mirror-reject caught the sender's `remove` but the receiver's `add` had already landed; the actual close shipped v1.7.802 as a server-atomic trade after a second audit pass exposed the asymmetry). V-B/V-C/V-D/V-E closed v1.7.745-746 as originally documented. Secondary surfaces (cp/exp/level/jobs/spells/jobLevels) extended via the PvE arbiter rewrite (v1.7.779-783, `docs/PVE-REWRITE-PLAN.md`). Mirror hardened across the v1.7.787-794 audit arc + a v1.7.796-805 follow-on pass: `mirrorApplyInvEvent` rejects `itemId=0` for `add`/`remove`; `inv-event` has per-kind rate cap; `give-item` honors `NON_TRADEABLE_ITEM_TYPES`; **`inv-state` push strips non-wire-managed fields (v1.7.796)** so stale mirror snapshots stop clobbering the client's authoritative `cp` / `exp` / `unlockedJobs` / `knownSpells` / `jobLevels`; **`mirrorSyncFromSave` first-save detect (v1.7.800)** seeds inventory + equipment + gil from save when no `inv_economies` row exists; **`equip` handler refuses to write 0s into missing rows (v1.7.800)**; **chest mimic routed through `createMimicBattle` (v1.7.804)**. **Equip-ownership check is still open** — cheater can `equip` items they don't own and inflate `mirrorReadEquippedBroadcast`. Needs an atomic `equip-from-inv` wire kind to avoid the race with the existing paired `remove`+`equip` events. See `[[ff3mmo-dup-vectors]]`. This doc is preserved as the long-form architectural reference for the audit + rollout.
