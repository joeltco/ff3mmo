...
> **⚠ Co-op party-encounter sync is BROKEN as of v1.7.472** (2026-05-18). HP / damage / state diverges between phones in party encounters from round 1; user confirmed in live testing that the v1.7.458→v1.7.472 fix band made it worse, not better. The per-turn RNG reseed (v1.7.468) is the only piece worth keeping; everything else in the band is suspect. Next-session candidates: full revert of v1.7.458→v1.7.472 OR a server-arbitrated turn-outcome rewrite. Solo + boss + PvP duel paths are unaffected. See memory `project_ff3mmo_coop_sync_2026_05_18.md` for the post-mortem.
