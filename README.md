...
> **Co-op sync status (v1.7.473, 2026-05-18):** Host-authoritative deltas rewrite shipped flag-off behind `COOP_HOST_ARB` (in `src/coop-resolver.js`). Phases 0-8 complete; commits `19f1403` → `6d05aac`. **Live behavior is byte-identical to v1.7.472** — the broken lockstep path still runs in prod until the flag flips. Two-phone smoke pending; cutover procedure in [`docs/COOP-PHASE-6-SMOKE.md`](docs/COOP-PHASE-6-SMOKE.md). Solo + boss + PvP duel paths unaffected. Architecture: [`MULTIPLAYER.md`](MULTIPLAYER.md) "host-authoritative model" section. Full plan: [`docs/COOP-REWRITE-PLAN.md`](docs/COOP-REWRITE-PLAN.md).
