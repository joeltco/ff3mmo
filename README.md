...
> **Co-op battles REMOVED (v1.7.500, 2026-05-20):** party-member co-op random encounters + Battle Assist were ripped out after three architectures (lockstep / host-authoritative / viewer) all froze the guest phone. **Random encounters are solo-only.** PvP duels, party invites, chat, presence, give-item, roster all work and are unaffected. A from-scratch co-op rebuild is planned — failure history + root causes + design intent are in the `ff3mmo-coop-rebuild` auto-memory; the removed code is in git history before v1.7.500. The historical narrative below (1.7.398-1.7.456) describes co-op as it was when built — those features no longer exist.
