...
v1.7.386 — **Real multiplayer is live.** WebSocket presence (real players in the roster panel), real chat over the wire (world / party / pm), real PvP duels with hook-on-target's-next-encounter, server-relayed combat sync (seed + action relay, both 1v1 and party-PvP), real party invites with Z/X accept prompt + one-party-per-player enforcement, fake-roster mid-battle ally fills synced across clients, graceful opponent-flee + disconnect handling, mid-session profile sync for level-ups / equipment changes. Engine loop runs on a Web Worker tick (instead of `requestAnimationFrame`) so backgrounded tabs stay in sync with the server. Fake `PLAYER_POOL` is now empty — the world is real-player-only by default; archived as `_FAKE_POOL` for one-line re-enable. Full architecture in [MULTIPLAYER.md](MULTIPLAYER.md), per-deploy detail in `CHANGELOG.md` 1.7.366 → 1.7.386.
