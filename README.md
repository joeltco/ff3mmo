...
> **Co-op battles REMOVED (v1.7.500) + PvP battles DISABLED (v1.7.502), 2026-05-20:** both real-time two-client battle features are off, for the same root cause — client-side lockstep can't hold cross-phone determinism. Co-op random encounters + Battle Assist were ripped out after three architectures (lockstep / host-authoritative / viewer) all froze the guest phone; **random encounters are solo-only.** PvP duels were then disabled behind a flag (`PVP_ENABLED`, server + client) after live battles desynced completely (turn order, damage, end-of-battle all diverged). PvP code is left in place for a planned rewrite to an **authoritative-host** model (one side computes outcomes, the other renders). **Party invites, chat, presence, give-item, roster all still work and are unaffected.** Failure history + design intent: `ff3mmo-coop-rebuild` + `ff3mmo-pvp-disabled` auto-memories; removed/disabled code is in git history. The historical narrative below (1.7.398-1.7.456) describes co-op + PvP combat as they were when built.
