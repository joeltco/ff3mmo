...
**Status as of v1.7.794:**

- **PvE solo combat** — fully server-validated (encounter rewards exp/gil/cp/drop, shops, chests, vases) via the **PvE arbiter + economy arbiter** arc shipped v1.7.771-783, hardened v1.7.787-794. See `docs/PVE-REWRITE-PLAN.md` (incl. "Post-arc audit" section) + `design-notes.md#pve-arbiter-economy-validation`. Replay-validate model: client runs battle locally with server-chosen monsters + seed, server outcome-validates at battle-end. All four flags (`PVE_ARBITER` + `SERVER_ECONOMY`, server + client) LIVE. `pve-encounter-request` gates `zoneKey` against `entry.loc` (`_LOC_ZONE_ALLOWLIST` in `ws-presence.js`); per-kind rate caps on chest/vase/shop/PvE wires bound the dungeon-chest replay exploit reopened by v1.7.789.
