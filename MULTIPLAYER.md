...
**Status as of v1.7.805:**

- **PvE solo combat** — fully server-validated (encounter rewards exp/gil/cp/drop, shops, chests, vases, **chest mimics**) via the **PvE arbiter + economy arbiter** arc shipped v1.7.771-783, hardened v1.7.787-794, and extended v1.7.796-805 (chest mimic via `createMimicBattle`, server-atomic trade, mirror seed gap closure). See `docs/PVE-REWRITE-PLAN.md` (incl. "Post-arc audit" section) + `design-notes.md#pve-arbiter-economy-validation`. Replay-validate model: client runs battle locally with server-chosen monsters + seed, server outcome-validates at battle-end. All four flags (`PVE_ARBITER` + `SERVER_ECONOMY`, server + client) LIVE. `pve-encounter-request` + chest-mimic claim both gate `zoneKey` against `entry.loc` (`_LOC_ZONE_ALLOWLIST` in `ws-presence.js`); per-kind rate caps on chest/vase/shop/PvE wires bound the dungeon-chest replay exploit reopened by v1.7.789.
...
- **Mirror integrity sweep (v1.7.993-995)** — instrumented every path that changes a mirrored resource instead of reading for them. Three real desync sources found: `_equipOptimum` never emitted for the offhand it released (the only one of four `releaseOffhandForTwoHanded` call sites that ignored the return), the quiver-empty clear mutated a hand slot silently, and quest gil never reached the server at all. All were "the player looks right to themselves and wrong to everyone else", which is why none was ever reported. ⚠ Equipment moves through `setEquipSlotId` **and** by direct `ps.weaponR = ...` writes — both bugs were the second kind. ⚠ Silence is CORRECT under `SERVER_ECONOMY` / `PVE_ARBITER`, where the server is sole writer; a naive "must emit" rule flags those and invites double-counting. Gated by `check-equip-emit.mjs` / `check-inv-emit.mjs` / `check-save-lockstep.mjs`; `[update divergence]` carries the client build since v1.7.992.
- **Inventory mirror (v1.7.740-746, extended v1.7.796-804)** — server-canonical gil + inventory + equipped state. Prerequisite for both arbiter arcs above. All 5 dup vectors closed (V-A was a partial close in v1.7.745, **fully closed v1.7.802** via server-atomic trade after a 2026-06-01 audit found the receiver's `add` always landed before the sender's `remove` could reject). See `[[ff3mmo-dup-vectors]]`.
- **Roster Trade (v1.7.598-616, atomic v1.7.802)** — real-MP item transfer with server type-whitelist + audit log. Since v1.7.802 the server applies the remove+add atomically via `mirrorApplyInvEvent` and pushes `inv-state` to both sides; clients no longer emit inv-events for trades. See `design-notes#roster-trade-real-multiplayer`.
