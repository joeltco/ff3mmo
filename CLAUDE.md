...
| Roster Trade (real-MP, item transfer) | — | `src/trade.js` (client lifecycle + net handlers via `setNetTrade*Handler`), `src/net.js` (`sendNetTradeOffer` / `sendNetTradeResponse` / `sendNetTradeCancel`), `ws-presence.js` (`case 'trade-offer'` / `'trade-response'` / `'trade-cancel'` + `_pendingTrades` Map + disconnect cleanup). Sender's `tickTrade` only runs timeout/death-cancel — server result drives accept. Receiver prompt via `showMsgBoxPrompt` mirrors party-invite busy guard. **Trust limitation:** server doesn't validate sender owns the claimed item (same as `give-item`); dup-on-claim possible. v1.7.598. |
