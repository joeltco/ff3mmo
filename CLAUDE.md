...
| `check-battle-bg` | the battle backdrop stays the cartridge's: the SHIPPED renderer compared **pixel for pixel** against a live-PPU capture of all 24 backdrops (`tools/monscan/battle-bg-sweep.json`); BOTH map lookup tables read (maps 256-511 live in the second one); all 512 maps in range; overworld terrain -> world tile-prop **byte 2**; and — the failure this was written for — that the battle screen actually CALLS the drawer. Doc: `docs/BATTLE-BACKDROPS.md` |
...
| `tools/battle-shot.mjs` | the BATTLE VIEWPORT through the shipped drawers — `--map N`, `--world X Y`, `--bg N`, `--all out.png`. There was no way to see this screen without walking into a fight, which is exactly how the battle backdrop stayed decoded-but-undrawn |
| `tools/battle-bg-sheet.mjs` | all 24 battle backdrops, labelled with the maps that use each. `--all` on battle-shot puts monsters in front of them |
